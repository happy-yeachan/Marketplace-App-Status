...
이 대시보드는 모든 앱의 실시간 상태를 한 화면에 집약합니다. 응답 시간, 히트 히스토리, 즉각적인 장애 알림까지 — Atlassian 인스턴스에 손대지 않고.
...
│       └── marketplace/
│           ├── search/
│           │   └── route.ts        # GET — Marketplace 검색 프록시 + 자동 탐색
│           └── popular/
...
Atlassian Marketplace REST API v2 프록시. 정적 맵에 없는 벤더는 자동 탐색으로 보완 (쿼리당 최대 12개 벤더). 결과는 60초 인메모리 캐시.
...
환경변수 설정 불필요. Atlassian Marketplace REST API v2와 벤더 상태 페이지만 사용합니다.
