...
**Marketplace App Status**는 모든 앱의 실시간 상태를 한 화면에 집약합니다. 응답 시간, 히트 히스토리, 즉각적인 장애 알림까지 — Atlassian 인스턴스에 손대지 않고요.

<div align="center">
  <img width="100%" alt="Marketplace App Status 대시보드" src="https://github.com/user-attachments/assets/c5c03736-ee9e-469b-ba1a-d933c67348a0" />
  <p><strong>여러 Marketplace 앱의 실시간 상태를 한 화면에서 확인</strong><br>
  <em>Outage와 Degraded 상태를 색상으로 바로 파악하고, 원클릭으로 벤더 상태페이지 이동</em></p>
</div>
...
| **Marketplace 검색** | 앱 이름으로 검색, 페이지당 12개씩 페이지네이션. 상태 URL을 서버 사이드에서 자동 해석. |
| **신규 사용자 기본 앱** | 첫 방문 시 인기 앱 6개를 자동으로 추가해 바로 유용하게 사용 가능. 공유 링크로 진입 시에는 건너뜀. |
| **Atlassian 플랫폼 상태** | Atlassian 제품 23개(Jira, Confluence, Bitbucket, Loom, Rovo 등)의 실시간 상태를 별도 배너로 표시. 각 제품의 개별 Statuspage API를 병렬로 조회. |
...
| **자동 갱신** | 설정 가능한 간격으로 상태 체크 자동 실행: Off / 1m / 5m / 15m. 기본값 5분. |
| **상태 변경 알림** | 앱이 정상 / 성능 저하 / 장애 간 전환될 때 인앱 토스트 알림. 선택적으로 브라우저 알림(Notification 권한)을 활성화해 탭이 백그라운드에 있어도 즉시 알림 수신 가능. |
...
| **Confluence 임베드** | Confluence Cloud 페이지에 iframe으로 삽입 가능(`*.atlassian.net`). CSP `frame-ancestors`로 허용 설정. `X-Frame-Options`는 와일드카드를 지원하지 않아 의도적으로 제외. |
...
│       ├── atlassian-platform/
│       │   └── route.ts            # GET — Atlassian 제품 23개 상태 (병렬 조회)
│       └── marketplace/
│           ├── search/
...
│   ├── add-app-dialog.tsx          # 검색 기반 앱 추가 플로우 (페이지네이션 포함)
...
│   ├── site-footer.tsx             # 피드백 및 법적 링크가 담긴 푸터
...
2. **토큰 점수 매칭** — 앱 이름을 토큰화(불용어 및 "jira" 같은 플랫폼 단어 제외)하고, 각 컴포넌트에 키워드·플랫폼 중복 점수를 매깁니다. 리프 컴포넌트는 부모 그룹 이름을 상속해 점수를 계산합니다.
...
**신규 사용자 동작:** 공유 링크로 처음 방문한 사용자는 기본 앱 자동 추가를 건너뜁니다 — 가져오기 확인 후 공유된 앱만 추가됩니다.
...
Atlassian 제품 23개의 실시간 상태를 병렬로 조회합니다. 통합 `summary.json`은 사용하지 않습니다 — 대규모 장애 시 수치가 지연되는 것이 확인되어, 각 제품의 개별 Statuspage API를 직접 조회합니다.
...
`overallIndicator`는 개별 제품 중 가장 심각한 상태. 60초 캐시(`s-maxage=60, stale-while-revalidate=120`).
...
`offset` 파라미터로 페이지네이션 지원 — 앱 추가 다이얼로그에서 페이지당 12개를 로드하며 "더 보기" 버튼으로 다음 페이지를 추가합니다.

결과는 CDN 레이어에서 60초 캐시(`s-maxage=60, stale-while-revalidate=120`).
...
서버 사이드 API 라우트를 위해 Next.js 호환 호스트가 필요합니다(Vercel 권장). GitHub Pages, Cloudflare Pages(Workers 없는 순수 정적)는 지원되지 않습니다.
...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `NEXT_PUBLIC_SITE_URL` | `https://marketplace.yeachan.cloud` | 캐노니컬 URL, OG 태그, 사이트맵에 사용 |
...
**통합 Atlassian summary 대신 개별 제품 페이지를 조회하는 이유:** 2026년 5월 Atlassian 대규모 장애 당시 통합 `summary.json`이 `indicator: "none"`을 반환하는 동안 개별 제품 20여 개는 이미 장애를 표시하고 있었습니다. 통합 API는 지연이 있고 개별 제품 Statuspage가 신뢰할 수 있는 원본 소스입니다.
